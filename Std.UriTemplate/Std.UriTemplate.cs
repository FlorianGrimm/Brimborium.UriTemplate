...
public class UriTemplate {
                    ...
                    if (toToken) {
                        if (op == null) {
                            ...
                            if (char.IsDigit(character)) {
                                _ = maxCharBuffer.Append(character);
                            } else {
    ...
    private static void AddExpandedValue(string prefix, object value, StringBuilder result, int maxChar, bool replaceReserved) {
        ...
        _ = result.EnsureCapacity(max * 2); // hint to SB
        ...
        for (int i = 0; i < stringValue.Length && charCount < max; i++) {
            ...
            if (toReserved) {
                _ = reservedBuffer.Append(toAppend);
                ...
                if (reservedBuffer.Length == 3) {
                    ...
                    if (isEncoded) {
                        _ = result.Append(reservedBuffer);
                    } else {
                        _ = result.Append("%25");
                        // only if !replaceReserved
                        _ = result.Append(reservedBuffer.ToString(1, 2));
                    }
                    toReserved = false;
                    _ = reservedBuffer.Clear();
                }
        ...
        if (toReserved) {
            _ = result.Append("%25");
            _ = result.Append(reservedBuffer.ToString(1, reservedBuffer.Length - 1));
        }
    }
    ...
    private static bool ExpandToken(
        ...
        object value;
        _ = substitutions.TryGetValue(token, out value);
        ...
        switch (substType) {
            case SubstitutionType.STRING:
                _ = AddStringValue(op, token, value, result, maxChar);
                break;
            case SubstitutionType.LIST:
                _ = AddListValue(op, token, (IList)value, result, maxChar, composite);
                break;
            case SubstitutionType.DICTIONARY:
                _ = AddDictionaryValue(op, token, ((IDictionary)value), result, maxChar, composite);
                break;
        }
